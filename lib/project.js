...
import {default as redis, hydrateObject, prepareObject} from './redis.js'
...
export async function createProject() {
  ...
  return {id, status, token, createdAt, updatedAt, userParams, processing: {}, files: []}
}
...
export async function addFileToProject(id, fileId) {
  await redis.sadd(`project:${id}:files`, fileId)
  await touchProject(id)
}

export async function getProjectFiles(id) {
  return redis.smembers(`project:${id}:files`)
}
...
export async function setProjectPipeline(id, pipeline) {
